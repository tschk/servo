    ...
    Alt::Connection: ConnectionInterface<Device = Alt>,
{
    ...
    pub fn new() -> Result<Connection<Def, Alt>, Error> {
        eprintln!("[sol-servo] SURFMAN_SELECTOR_MARKER_20260422 default start");
        eprintln!("[sol-servo] surfman.multi.connection.new default start");
        match <Def::Connection>::new() {
            Ok(connection) => {
                eprintln!("[sol-servo] surfman.multi.connection.new default done");
                Ok(Connection::Default(connection))
            },
            Err(_) => {
                eprintln!("[sol-servo] surfman.multi.connection.new alternate start");
                <Alt::Connection>::new().map(|connection| {
                    eprintln!("[sol-servo] surfman.multi.connection.new alternate done");
                    Connection::Alternate(connection)
                })
            },
        }
    }
    ...
    pub fn from_display_handle(
        ...
        eprintln!("[sol-servo] surfman.multi.connection.from_display_handle default start");
        match <Def::Connection>::from_display_handle(handle) {
            Ok(connection) => {
                eprintln!("[sol-servo] surfman.multi.connection.from_display_handle default done");
                Ok(Connection::Default(connection))
            },
            Err(_) => {
                eprintln!("[sol-servo] surfman.multi.connection.from_display_handle alternate start");
                <Alt::Connection>::from_display_handle(handle).map(|connection| {
                    eprintln!("[sol-servo] surfman.multi.connection.from_display_handle alternate done");
                    Connection::Alternate(connection)
                })
            },
        }
