...
impl Connection {
    ...
    pub fn new() -> Result<Connection, Error> {
        unsafe {
            eprintln!("[sol-servo] surfman.x11.connection.new xlib open start");
            let xlib = Xlib::open().map_err(|_| Error::ConnectionFailed)?;
            eprintln!("[sol-servo] surfman.x11.connection.new xlib open done");

            X_THREADS_INIT.call_once(|| {
                eprintln!("[sol-servo] surfman.x11.connection.new XInitThreads");
                (xlib.XInitThreads)();
            });

            eprintln!("[sol-servo] surfman.x11.connection.new XOpenDisplay start");
            let x11_display = (xlib.XOpenDisplay)(ptr::null());
            eprintln!("[sol-servo] surfman.x11.connection.new XOpenDisplay done");
            if x11_display.is_null() {
                return Err(Error::ConnectionFailed);
            }

            eprintln!("[sol-servo] surfman.x11.connection.new create_egl_display start");
            let egl_display = create_egl_display(x11_display)?;
            eprintln!("[sol-servo] surfman.x11.connection.new create_egl_display done");
...
unsafe fn create_egl_display(display: *mut Display) -> Result<EGLDisplay, Error> {
    EGL_FUNCTIONS.with(|egl| {
        ...
        eprintln!("[sol-servo] surfman.x11.create_egl_display Initialize start");
        let ok = egl.Initialize(egl_display, &mut egl_major_version, &mut egl_minor_version);
        eprintln!("[sol-servo] surfman.x11.create_egl_display Initialize done");
        if ok == egl::FALSE {
            let fallback_display = egl.GetDisplay(display as *mut c_void);
            if fallback_display != egl::NO_DISPLAY {
                eprintln!("[sol-servo] surfman.x11.create_egl_display fallback Initialize start");
                let retry_ok = egl.Initialize(
                    fallback_display,
                    &mut egl_major_version,
                    &mut egl_minor_version,
                );
                eprintln!("[sol-servo] surfman.x11.create_egl_display fallback Initialize done");
                if retry_ok != egl::FALSE {
