    ...
    pub fn new() -> Result<Connection, Error> {
        unsafe {
            EGL_FUNCTIONS.with(|egl| {
                eprintln!("[sol-servo] surfman.surfaceless.connection.new GetPlatformDisplay start");
                ...
                eprintln!("[sol-servo] surfman.surfaceless.connection.new GetPlatformDisplay done");
                if egl_display == egl::NO_DISPLAY {
                    return Err(Error::ConnectionFailed);
                }

                let (mut egl_major_version, mut egl_minor_version) = (0, 0);
                eprintln!("[sol-servo] surfman.surfaceless.connection.new Initialize start");
                let ok =
                    egl.Initialize(egl_display, &mut egl_major_version, &mut egl_minor_version);
                eprintln!("[sol-servo] surfman.surfaceless.connection.new Initialize done");
                if ok == egl::FALSE {
                    return Err(Error::ConnectionFailed);
                }
